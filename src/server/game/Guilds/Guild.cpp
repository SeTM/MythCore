...
void Guild::Member::WritePacket(WorldPacket& data) const
{
    ...
    if(m_officerNote.c_str() && strlen(m_officerNote.c_str()) > 0)
        data << m_officerNote;
    else
        data << "";
}
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    if (!pFrom->IsBank() && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE) && m_pPlayer->GetSession()->GetSecurity() > SEC_VIP)       // TODO: move to scripts
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    if (player->GetSession()->GetSecurity() > SEC_VIP && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
    {
