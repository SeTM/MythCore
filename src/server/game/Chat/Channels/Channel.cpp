...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    if (plr)
    {
        if (HasFlag(CHANNEL_FLAG_LFG) &&
            sWorld->getBoolConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && plr->GetSession()->GetSecurity() <= SEC_VIP && plr->GetGroup())
        {
            MakeNotInLfg(&data);
            SendToOne(&data, p);
            return;
...
void Channel::List(Player* player)
{
    ...
    else
    {
        ...
        for (PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
        {
            ...
            if (plr && (player->GetSession()->GetSecurity() > SEC_VIP || plr->GetSession()->GetSecurity() <= AccountTypes(gmLevelInWhoList)) &&
