...
void WardenWin::Init(WorldSession *pClient, BigNumber *K)
{
    ...
    */
    ...
    sLog->outStaticDebug("WARDEN: Server side warden for client %u initializing...", pClient->GetAccountId());
    PrintHexArray("  C->S Key: ", InputKey, 16, true);
    PrintHexArray("  S->C Key: ", OutputKey, 16, true);
    PrintHexArray("  Seed: ", Seed, 16, true);
    sLog->outStaticDebug("WARDEN: Loading Module...");
...
{
    sLog->outStaticDebug("WARDEN: Initialize module");
...
{
    sLog->outStaticDebug("WARDEN: Request hash");
...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    sLog->outStaticDebug("WARDEN: Request hash reply: succeed");
...
{
    sLog->outStaticDebug("WARDEN: Request data");
    ...
    for (int i = 0; i < 5; ++i)                             // for now include 3 MEM_CHECK's
    {
        if (MemCheck.empty())
            break;
        id = MemCheck.back();
    ...
    for (int i = 0; i < 10; ++i)                             // for now include 5 random checks
    {
        id = irand(1, maxid - 1);
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
        stream << *itr << " ";
    sLog->outStaticDebug("WARDEN: %s",stream.str().c_str());
}

void WardenWin::HandleData(ByteBuffer &buff)
{
    sLog->outStaticDebug("WARDEN: Handle data");
    ...
    //TIMING_CHECK
    {
        ...
        sLog->outStaticDebug("WARDEN: ServerTicks %u", ticksNow);         // now
        sLog->outStaticDebug("WARDEN: RequestTicks %u", ServerTicks);     // at request
        sLog->outStaticDebug("WARDEN: Ticks %u", newClientTicks);         // at response
        sLog->outStaticDebug("WARDEN: Ticks diff %u", ourTicks - newClientTicks);
    }
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    sLog->outStaticDebug("WARDEN: RESULT MEM_CHECK not 0x00, CheckId %u account Id %u", *itr, Client->GetAccountId());
                    found = true;
                    checkNum = *itr;
                ...
                buff.rpos(buff.rpos() + rd->Length);
                sLog->outStaticDebug("WARDEN: RESULT MEM_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                buff.rpos(buff.rpos() + 1);
                if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                    sLog->outStaticDebug("WARDEN: RESULT PAGE_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                else if (type == MODULE_CHECK)
                    sLog->outStaticDebug("WARDEN: RESULT MODULE_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                else if (type == DRIVER_CHECK)
                    sLog->outStaticDebug("WARDEN: RESULT DRIVER_CHECK passed CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
            case LUA_STR_CHECK:
            {
                ...
                if (luaStrLen != 0)
                {
                    ...
                    sLog->outStaticDebug("WARDEN: Lua string: %s", str);
                    delete[] str;
                }
                buff.rpos(buff.rpos() + luaStrLen);         // skip string
                sLog->outStaticDebug("WARDEN: RESULT LUA_STR_CHECK passed, CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
            case MPQ_CHECK:
            {
                ...
                buff.rpos(buff.rpos() + 20);                // 20 bytes SHA1
                sLog->outStaticDebug("WARDEN: RESULT MPQ_CHECK passed, CheckId %u account Id %u", *itr, Client->GetAccountId());
                break;
            }
