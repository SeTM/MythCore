...
{
    sLog->outStaticDebug("WARDEN: Send module to client");
...
{
    sLog->outStaticDebug("WARDEN: Request module");
...
*/
        if (m_WardenCheckTimer > 0)
        {
            if (diff >= m_WardenCheckTimer)
            {
                RequestData();
                // 25-35 second
                m_WardenCheckTimer = irand(5, 15) * IN_MILLISECONDS;
            }
            else
...
bool WardenBase::IsValidCheckSum(uint32 checksum, const uint8 *Data, const uint16 Length)
{
    ...
    {
        sLog->outStaticDebug("WARDEN: CHECKSUM IS VALID");
        return true;
    }
}
...
void WorldSession::HandleWardenDataOpcode(WorldPacket & recv_data)
{
    ...
    uint8 Opcode;
    recv_data >> Opcode;
    sLog->outStaticDebug("WARDEN: Got packet, opcode %02X, size %u", Opcode, recv_data.size());
    recv_data.hexlike();
    ...
    switch(Opcode)
    {
        ...
        case WARDEN_CMSG_MEM_CHECKS_RESULT:
            sLog->outStaticDebug("WARDEN: NYI WARDEN_CMSG_MEM_CHECKS_RESULT received!");
            break;
        ...
        case WARDEN_CMSG_MODULE_FAILED:
            ...
            break;
        default:
            sLog->outStaticDebug("WARDEN: Got unknown warden opcode %02X of size %u.", Opcode, recv_data.size() - 1);
            break;
    }
}
