...
void ObjectMgr::LoadAntiCheatConfig()
{
    ...
    if (!result)
    {
        ...
        sLog->outString();
        sLog->outString(">> Loaded %u GameGaurd config definitions", count);
        sLog->outError("Error loading `anticheat_config` table or table is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError("Wrong check type id %u in `anticheat_config` table, ignoring.",AntiCheatConfigEntry.checkType);
            continue;
        }
        ...
        AntiCheatConfigEntry.description  = fields[5+ANTICHEAT_CHECK_PARAMETERS*2+ANTICHEAT_ACTIONS*2].GetString();
