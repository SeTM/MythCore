...
struct DefaultTargetSelector : public std::unary_function<Unit* , bool>
{
    ...
    DefaultTargetSelector(Unit const* pUnit, float dist, bool playerOnly, int32 aura,SelectArc targettype = SELECT_ARC_NONE, float arc = 0.0f) : me(pUnit)
    ...
    bool operator() (Unit const* pTarget)
    {
        ...
        if(m_arc_type)
        {
            ...
            else if(m_arc_type == SELECT_ARC_BACK)
                if(me->HasInArc(2 * static_cast<float>(M_PI) - m_arc, pTarget))
                    return false;
