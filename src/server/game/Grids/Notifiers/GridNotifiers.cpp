...
void PlayerRelocationNotifier::Visit(CreatureMapType &m)
{
    ...
    for (CreatureMapType::iterator iter=m.begin(); iter != m.end(); ++iter)
    {
        ...
        if (c && relocated_for_ai && !c->isNeedNotify(NOTIFY_VISIBILITY_CHANGED))
...
void CreatureRelocationNotifier::Visit(PlayerMapType &m)
{
    ...
    for (PlayerMapType::iterator iter=m.begin(); iter != m.end(); ++iter)
    {
        ...
        if (pl && pl->isAlive() && !pl->isInFlight() && !pl->m_seer->isNeedNotify(NOTIFY_VISIBILITY_CHANGED))
...
void CreatureRelocationNotifier::Visit(CreatureMapType &m)
{
    ...
    for (CreatureMapType::iterator iter=m.begin(); iter != m.end(); ++iter)
    {
        ...
        if (c && &i_creature && c != &i_creature && c->isAlive() && !c->isNeedNotify(NOTIFY_VISIBILITY_CHANGED))
