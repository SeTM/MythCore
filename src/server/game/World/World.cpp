...
void World::LoadConfigSettings(bool reload)
{
    ...
    //Premium Accounts
    m_bool_configs[CONFIG_PREMIUM_ENABLE]       = sConfig->GetBoolDefault("Premuim.Account.Enable", false);
    m_int_configs[CONFIG_PREMIUM_GMLEVEL]       = sConfig->GetIntDefault("Premuim.Account.GmLevel", 1);
    rate_values[RATE_XP_PREMIUM_KILL]           = sConfig->GetFloatDefault("Premium.Rate.XP.Kill", 1.0f);
    rate_values[RATE_XP_PREMIUM_QUEST]          = sConfig->GetFloatDefault("Premium.Rate.XP.Quest", 1.0f);
    rate_values[RATE_XP_PREMIUM_EXPLORE]        = sConfig->GetFloatDefault("Premium.Rate.XP.Explore", 1.0f);
    rate_values[RATE_PREMIUM_REPUTATION_GAIN]   = sConfig->GetFloatDefault("Premium.Rate.Reputation.Gain", 1.0f);
    rate_values[RATE_PREMIUM_HONOR]             = sConfig->GetFloatDefault("Premium.Rate.Honor", 1.0f);
    ...
    m_int_configs[CONFIG_GM_LEVEL_ALLOW_ACHIEVEMENTS]  = sConfig->GetIntDefault("GM.AllowAchievementGain.Level", 0);
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outString( "Loading Myth Anti-Cheating System..." );
    ...
    sLog->outString("Loading Myth Anti-Cheating System Data..." );
    ...
    sLog->outString("Loading GameGuard.Warden Data..." );
    WardenDataStorage.Init();
...
void World::SendGlobalMessage(WorldPacket* packet, WorldSession* self, uint32 team, AccountTypes sec)
{
    SessionMap::const_iterator itr;
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
/// Send a System Message to all players with equal or higher GMlevel (except self if mentioned)
void World::SendWorldTextWithSecurity(AccountTypes sec, int32 string_id, ...)
{
    va_list ap;
    va_start(ap, string_id);
