...
int WorldSocket::HandleAuthSession (WorldPacket& recvPacket)
{
    ...
    uint32 premlvl = sWorld->getIntConfig (CONFIG_PREMIUM_GMLEVEL); //Select VIP level
    if (premlvl <= security && sWorld->getBoolConfig(CONFIG_PREMIUM_ENABLE)) // check for premium acc and system enable
        isPremium = true; 
