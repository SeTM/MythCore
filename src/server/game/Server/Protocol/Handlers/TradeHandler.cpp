    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
                ...
                if (_player->GetSession()->GetSecurity() > SEC_VIP && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
            ...
            if (hisItems[i])
            {
                ...
                if (trader->GetSession()->GetSecurity() > SEC_VIP && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
