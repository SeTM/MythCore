...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    if (heroic_free_slots == 0 && GetSecurity() <= SEC_VIP && class_ == CLASS_DEATH_KNIGHT)
    {
    ...
    bool AllowTwoSideAccounts = !sWorld->IsPvPRealm() || sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_ACCOUNTS) || GetSecurity() > SEC_VIP;
    ...
    if (!AllowTwoSideAccounts || skipCinematics == 1 || class_ == CLASS_DEATH_KNIGHT)
    {
        ...
        if (result2)
        {
            ...
            if (GetSecurity() <= SEC_VIP && class_ == CLASS_DEATH_KNIGHT)
            {
    ...
    if (GetSecurity() <= SEC_VIP && class_ == CLASS_DEATH_KNIGHT && !have_req_level_for_heroic)
    {
        data << (uint8)CHAR_CREATE_LEVEL_REQUIREMENT;
        SendPacket(&data);
        return;
