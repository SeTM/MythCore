...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
    ...
    if (!accountBound && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_MAIL) && pl->GetTeam() != rc_team && GetSecurity() <= SEC_VIP)
    {
    ...
    if (items_count > 0 || money > 0)
    {
        ...
        if (money > 0 &&  GetSecurity() > SEC_VIP && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
        {
