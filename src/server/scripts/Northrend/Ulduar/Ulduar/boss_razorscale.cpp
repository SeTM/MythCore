...
public:
    ...
    struct boss_razorscaleAI : public BossAI
    {
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            ...
            if (id == POINT_FLIGHT)
                if (Creature* pFacTarget = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_EXP_COMMANDER)))
                    me->SetFacingToObject(pFacTarget);
        }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        case EVENT_FLIGHT:
                            ...
                            me->AttackStop();
                            me->RemoveAllAuras();
                            me->GetMotionMaster()->MovePoint(POINT_FLIGHT, RazorFlight);
    ...
    struct npc_mole_machine_triggerAI : public Scripted_NoMovementAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (SummonTimer <= uiDiff)
            {
                ...
                if (!(rand()%2))
                {
                    if(Creature * pSummon = me->SummonCreature(NPC_DARK_RUNE_WATCHER, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 3000))
                        pSummon->Attack(pSummon->getVictim(), true);
                    uint8 random = urand(1,2);
                    for (uint8 i = 0; i < random; ++i)
                        if(Creature * pSummon = me->SummonCreature(NPC_DARK_RUNE_GUARDIAN, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 3000))
                            pSummon->Attack(pSummon->getVictim(), true);
                }
                else if(Creature * pSummon = me->SummonCreature(NPC_DARK_RUNE_SENTINEL, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 3000))
                    pSummon->Attack(pSummon->getVictim(), true);
