...
class svalna_temp : public CreatureScript
{
public:
    svalna_temp() : CreatureScript("svalna_temp") { }
    struct svalna_tempAI : public ScriptedAI
    {
        svalna_tempAI (Creature *c) : ScriptedAI(c) {Reset();}

        void Reset() { }

        void JustDied(Unit* /*killer*/)
        {
            GameObject* door = me->FindNearestGameObject(201375, 100.0f);
            if (door)
    ...
    {
        return new svalna_tempAI(pCreature);
    }
};
