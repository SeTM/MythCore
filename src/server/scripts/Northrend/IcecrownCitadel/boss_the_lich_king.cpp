    ...
    public:
        ...
        struct boss_the_lich_kingAI : public BossAI
        {
            ...
            void JustDied(Unit* /*pKiller*/)
            {
                ...
                Cleanup();
                instance->DoCastSpellOnPlayers(SPELL_PLAY_MOVIE);
                    ...
                    if (eventId == EVENT_CHECK_ALIVE_PLAYERS)
                    {
                        if (Creature *pTirion = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_TIRION)))
                        {
                            ...
                            for (TPlayerList::iterator it = players.begin(); it != players.end(); ++it)
                                if ((*it) && (*it)->GetDistanceZ(pTirion) > 10.0f && !(*it)->HasAura(SPELL_IN_FROSTMOURNE_ROOM))
                                {
                                    (*it)->RemoveAllAuras();
                    ...
                    if (uiEndingTimer <= uiDiff)
                    {
                        switch(uiStage)
                        {
                            case 1:
                            {
                                ...
                                TPlayerList players = GetPlayersInTheMap(me->GetMap());
                                for (TPlayerList::iterator it = players.begin(); it != players.end(); ++it)
                                    if ((*it) && (*it)->HasAura(SPELL_IN_FROSTMOURNE_ROOM))
                                        (*it)->TeleportTo(631, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 3, 0);
                            ...
                            case 22:
                            {
                                ...
                                if(Creature* father = me->FindNearestCreature(NPC_TERENAS_MENETHIL, 100.0f))
                    ...
                    switch(uiStage)
                    {
                        case 1:
                        {
                            if(Creature* lich = Unit::GetCreature(*me, uiLichKingGUID))
                            {
                                ...
                                lich->GetMotionMaster()->MovePoint(1, 458.057281f, -2125.669678f, 1040.859985f);
                                me->SetFacingToObject(lich);
                            }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CHECK_PLAYER_IN_FROSTMOURNE_ROOM:
                    {
                        ...
                        {
                            if (Player *player = curVictim->ToPlayer())
                            {
                                uint8 count = 0;
...
class spell_lich_king_infection : public SpellScriptLoader
{
    public:
        ...
        class spell_lich_king_infection_AuraScript : public AuraScript
        {
            ...
            {
                amount = int32(1000.0f * powf(1.15f, float(uint32(aurEff->GetTickNumber()))));
            }
    ...
    public:
        ...
        class spell_lich_king_quake_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                caster->ClearUnitState(UNIT_STAT_CASTING);
            }
    ...
    public:
        ...
        class spell_lich_king_play_movie_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                AchievementEntry const *AchievLKK = GetAchievementStore()->LookupEntry(4530);
                if (!(target) || (AchievLKK && target->ToPlayer()->GetAchievementMgr().HasAchieved(AchievLKK)))
                    return;
    ...
    public:
        ...
        class spell_lich_king_harvest_soul_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const * aurEff, AuraEffectHandleModes mode)
            {
                ...
                if (!target || target->GetTypeId() != TYPEID_PLAYER || !caster)
                    return;
